...
def binary_classification_data(
    ...
    y = pd.Series(
        (np.random.rand(test_settings.N_SAMPLES) > 0.5).astype(int), name="target"
    )
    return X, y
...
def multiclass_classification_data(
    ...
    y = pd.Series(
        np.random.choice([0, 1, 2], size=test_settings.N_SAMPLES), name="target"
    )
    return X, y
...
def regression_data(test_settings: TestSettings) -> tuple[pd.DataFrame, pd.Series]:
    ...
    y = pd.Series(np.random.randn(test_settings.N_SAMPLES), name="target")
    return X, y
...
def mock_estimator() -> Mock:
    ...
    estimator.predict_proba.return_value = np.array(
        [[0.8, 0.2], [0.3, 0.7], [0.9, 0.1], [0.4, 0.6], [0.2, 0.8]]
    )
    estimator.feature_names_in_ = np.array(
...
def mock_transformer() -> Mock:
    ...
    transformer.fit_resample.return_value = (
        pd.DataFrame({"resampled": [1, 2, 3]}),
        pd.Series([0, 1, 0]),
    )
    return transformer
...
def logistic_regression_estimator(test_settings: TestSettings) -> LogisticRegression:
    ...
    return LogisticRegression(
        max_iter=200, random_state=test_settings.SEED, solver="liblinear"
    )
...
def random_forest_estimator(test_settings: TestSettings) -> RandomForestClassifier:
    ...
    return RandomForestClassifier(
        n_estimators=10, random_state=test_settings.SEED, max_depth=3
    )
...
if __name__ == "__main__":
    print("This file provides pytest fixtures.")
